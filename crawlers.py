...
class Crawler:
    ...
    def _get_addresses(self, timestamp):
        ...
        addresses = {
            service: list(sum(self._get_addresses_from_site(url, timestamp), ()))
    ...
    def get_queries(self, timestamp=None):
        ...
        raw_queries = [
            self.extraction_functions[key](value)
            for key, values in urls.items()
            for value in values
        ]

        tokenized_queries = [
            word_tokenize(phrase.lower().replace("\\n", "")) for phrase in raw_queries
        ]

        cleaned_queries = [
            word
            for word in set(sum(tokenized_queries, []))
...
class FirefoxCrawler(Crawler):
    def __init__(
        self,
        dir=os.path.join(os.environ["HOME"], ".personaldictionary"),
        profile="Profile0",
    ):
...
class ChromeCrawler(Crawler):
    def __init__(self, dir=os.path.join(os.environ["HOME"], ".personaldictionary")):
        ...
        self.db = os.path.join(
            os.environ["HOME"], ".config", "google-chrome", "Default", "History"
        )
        self.connected = False
